...
function Write-DatasheetCsv {
	...
	$folder = if ($leaf -ieq 'episode_datasheets') { $root } else { Join-Path $root 'episode_datasheets' }
...
function Start-TmdbEpisodeScrape {
	...
	while ($true) {
		...
		if ($ReturnToOrganiserOnComplete) {
			try {
				$scriptDir = $PSScriptRoot
				$root = if ((Split-Path -Leaf $scriptDir) -ieq 'episode_datasheets') { Split-Path -Parent $scriptDir } else { $scriptDir }
				$organiserPath = Join-Path $root 'episode_organiser.ps1'
				if (-not (Test-Path -LiteralPath $organiserPath)) {
					$organiserPath = Join-Path (Join-Path $root 'bin') 'episode_organiser.ps1'
				}
				if (-not (Test-Path -LiteralPath $organiserPath)) {
